...
(function (MessageType) {
    MessageType[MessageType["Authentication"] = 0] = "Authentication";
    MessageType[MessageType["Data"] = 1] = "Data";
    MessageType[MessageType["ConnectionStatus"] = 2] = "ConnectionStatus";
    MessageType[MessageType["HostshipRequest"] = 3] = "HostshipRequest";
    MessageType[MessageType["HostshipResignation"] = 4] = "HostshipResignation";
    MessageType[MessageType["NConnections"] = 5] = "NConnections";
    MessageType[MessageType["Disconnection"] = 6] = "Disconnection";
...
(function (ConnectionStatus) {
    ...
    ConnectionStatus[ConnectionStatus["ClientWithoutHost"] = 2] = "ClientWithoutHost";
    ConnectionStatus[ConnectionStatus["ClientWithHost"] = 3] = "ClientWithHost";
    ConnectionStatus[ConnectionStatus["Host"] = 4] = "Host";
...
function sendMessage(peer, messageType, data) {
    let buffer = new ArrayBuffer(2 + // OS header
...
function sendConnectionStatus(peer) {
    const currentHostName = currentHostId === null ? "" : peers[currentHostId].name;
    let buffer = new ArrayBuffer(Uint32Array.BYTES_PER_ELEMENT + // Status
...
function sendNumberConnections() {
    ...
    peers.forEach((peer) => { sendMessage(peer, MessageType.NConnections, buffer); });
}
function assignHost(newHost) {
    ...
    peers.forEach((peer) => { sendConnectionStatus(peer); });
}
function setToClient(peer) {
    ...
    socket.on('data', function (data) {
        ...
        switch (messageType) {
            case MessageType.Authentication: {
                if (Debug)
                    console.log(`Received Authentication from ${peer.id}: ${peer.name}`);
                ...
                offset += 4;
                const hostPassword = hostPasswordSize === 0 ?
                    "" :
            ...
            case MessageType.HostshipRequest: {
                ...
                if (password !== settings.hostPassword) {
                    ...
                    if (Debug)
                        console.log(`Provided: ${password}.  Is ${settings.hostPassword}`);
                    // We received the wrong password
                    return;
                }
